...
export const StageItem: React.FC<StageItemProps> = ({ 
  ...
  const progressStage = async (stageId: string) => {
    try {
      ...
      if (onError) {
        onError(error instanceof Error ? error.message : 'Failed to progress stage');
      }
    } finally {
  ...
  const cancelStage = async (stageId: string) => {
    try {
      ...
      if (onError) {
        onError(error instanceof Error ? error.message : 'Failed to cancel stage');
      }
    } finally {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
          <div className="stage-info">
            ...
            {showSoakTime && (
              <span className="stage-soak">soak {formatTimeHMS(planStage.soak_time)}</span>
            )}
          ...
          <>
            <div className="stage-timing">
              ...
              {releaseStage.state.startsWith('done_') && releaseStage.time_done && (
                <span className="stage-time">
                  Completed: {new Date(releaseStage.time_done).toLocaleString()}
                </span>
              )}
              {releaseStage.state.startsWith('done_') && releaseStage.time_elapsed !== undefined && (
                <span className="stage-time">
            ...
            {releaseStage.logs && (
              <div className="stage-logs-section">
                ...
                >
                  {logsExpanded ? '▼' : '▶'} Logs ({releaseStage.logs.split('\n').filter(line => line.trim()).length} entries)
                </button>
                {logsExpanded && (
                  <div 

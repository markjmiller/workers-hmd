...
export class SLOEvaluator {
  ...
  static evaluateSLOs(
    ...
    const summary = passed 
      ...
      : `${violations.length} of ${sloConfigs.length} SLO(s) violated: ${violations.map(v => 
   ...
   */
  private static getMetricValue(metrics: ObservabilityMetrics, percentile: SLOConfig['percentile']): number {
    switch (percentile) {
      case 'p999':
        return metrics.p999;
      case 'p99':
  ...
  static parseSLOsFromPlan(slos: any[]): SLOConfig[] {
    ...
    for (const slo of slos) {
      if (typeof slo === 'object' && slo.percentile && slo.latency_ms) {
        ...
        if (['p999', 'p99', 'p90', 'median'].includes(slo.percentile) &&
            typeof slo.latency_ms === 'number' && slo.latency_ms > 0) {
          parsedSLOs.push({
            percentile: slo.percentile,
